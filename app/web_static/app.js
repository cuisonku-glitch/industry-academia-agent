  ...
  return `
    <article class="paper-card">
      ...
      <div class="card-footer">
        <span class="report-state">${paper.has_local_report ? "已有精读底稿" : "尚未生成精读"}</span>
    ...
    <div class="detail-columns">
      <div>
        <section class="detail-panel">
          <div class="section-head"><h2>论文分析任务</h2><p>本地证据底稿不联网；Kimi 精读、图版解读和技术路线将在下一批接入。</p></div>
          <div class="action-grid">
            ...
            <button class="action-button" type="button" disabled><strong>Kimi 论文精读</strong><small>全文结构化总结＋页码引用，即将接入</small></button>
            <button class="action-button" type="button" disabled><strong>图版与公式解读</strong><small>提取原图、保留图号并调用视觉模型，即将接入</small></button>
            <button class="action-button" type="button" disabled><strong>技术路线与 draw.io</strong><small>生成可编辑节点、连线和框图，即将接入</small></button>
          </div>
      ...
      <section class="detail-panel">
        ...
        <div class="tab-row">
          <button class="tab-button active" data-detail-tab="report" type="button">精读底稿</button>
          <button class="tab-button" data-detail-tab="pdf" type="button" ${paper.has_pdf ? "" : "disabled"}>PDF 原文</button>
          <button class="tab-button" data-detail-tab="figures" type="button" disabled>论文图片</button>
...
async function showReport(paper) {
...
async function generateLocalReading() {
  ...
  try {
    ...
    $("#detail-content").innerHTML = `<article class="report-body">${renderMarkdown(result.report)}</article><div class="download-row"><a class="download-button" href="/api/papers/${paper.paper_id}/report?download=true">下载 Markdown</a></div>`;
